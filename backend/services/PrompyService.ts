...
חובה עליך לחלק כל תשובה ל-3 חלקים ברורים עם הכותרות הבאות:
1. **פתיחה:** - הקדמה קצרה שמושכת את תשומת הלב ומסבירה על מה נלמד.
2. **גוף השיעור:** - ההסבר המרכזי והמפורט של המושג, כולל דוגמאות במידת הצורך.
3. **סיום:** - סיכום קצרצר של מה שלמדנו או טיפ קטן לזכור את החומר.
אנא השתמש בכותרות המודגשות האלו בדיוק כך, כדי שהטקסט יהיה נוח לקריאה.` 
        },
        { 
          role: "user", 
          content: userPrompt 
